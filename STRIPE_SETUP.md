...
- The admin catalog shows every bundled and KV-only product. Because products are ordered from suppliers after a customer order, the admin tracks only **Dobavljivo po naročilu** or **Trenutno ni dobavljivo**, without maintaining stock quantities. Changes are stored in the Products KV namespace and the Worker blocks unavailable products during Checkout.

> **Security requirement:** protect both `/admin-panel.html` and `/api/admin/*` in the same Cloudflare Access application. The Worker rejects admin API requests without `Cf-Access-Authenticated-User-Email`, but that header is trustworthy only when Cloudflare Access is enabled in front of the route. Keep `/api/products` public because the storefront needs it; supplier prices and supplier links are removed from that public response, while the full catalog and all write operations remain under `/api/admin/*`.
